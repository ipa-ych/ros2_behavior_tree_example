...
{
    callStdSrvEmpty::callStdSrvEmpty(const std::string & condition_name, const BT::NodeConfig & conf)
    ...
    service_called_(false)
    {
        ...
        //                                                 std::bind(&callStdSrvEmpty::pong_callback, this, std::placeholders::_1),
        //                                                 options);
    ...
    callStdSrvEmpty::~callStdSrvEmpty()
    {
    ...
    BT::NodeStatus callStdSrvEmpty::tick()
    {
        ...
        RCLCPP_INFO(node_->get_logger(), "tick %s service ", service_name_);
        ...
        if (!service_called_)
        {
            ...
            {
                RCLCPP_ERROR(node_->get_logger(), "Service %s not available after waiting", service_name_);
        ...
        if (service_called_)
        {
            ...
            if (status == std::future_status::ready)
            {
                ...
                {
                    RCLCPP_INFO(node_->get_logger(), "Service %s called successfully", service_name_);
                    service_called_ = false;
                    return BT::NodeStatus::SUCCESS;
                }
                else
                {
                    RCLCPP_ERROR(node_->get_logger(), "Failed to call service %s", service_name_);
                    service_called_ = false;
                    return BT::NodeStatus::FAILURE;
                }
            }
